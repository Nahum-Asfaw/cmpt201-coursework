...
server_thread_args_t *pack_args(int cfd) { // lab8
  server_thread_args_t *args = malloc(sizeof(server_thread_args_t));
...
void *handle_client(void *arg) {
  ...
  for (;;) {
    // read, if read returns false, break
...
int main() {
  ...
  server_thread_args_t *thread_args[1000];
  for (int i = 0;; i++) {
    int cfd = accept(cfd, NULL, NULL);
    if (sfd == -1)
      handle_error("accept");

    thread_args[i] = pack_args(cfd);
    pthread_create(&server_threads[i], NULL, handle_client, thread_args[i]);
